...
const CryptoEducationCorner = () => {
    ...
    const learningPaths = [
      {
        title: "Crypto Beginner → Advanced (5 steps)",
        steps: [
    ...
    const cryptoTips = [
        ...
        },
        {
          ...
          readTime: "6 min read",
          cta: "Read Before You Buy Crypto",
        },
        {
        ...
        <div className="learning_path">
          {learningPaths.map((path) => (
            <div key={path.title} className="learning_path_card">
              <div className="learning_path_title">{path.title}</div>
              <div className="learning_path_steps">
                {path.steps.map((step, idx) => (
                  <div key={`${path.title}-${idx}`} className="learning_path_step">
                    <span className="learning_step_number">{idx + 1}</span>
                    <span className="learning_step_text">{step}</span>
                  </div>
                ))}
              </div>
            </div>
